    ...
    <div style={{ background: "#fff", border: "1px solid #ddd", padding: 8, borderRadius: 4 }}>
      <select
        ...
        onChange={(e) => {
          ...
          set("condition", newCond);
          if (["isempty", "isnotempty"].includes(newCond)) {
            set("value", "");
          }
        }}
      >
      ...
      {["isempty", "isnotempty"].includes(cond) ? null : type === "Int" && cond === "between" ? (
        <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
          <input
            ...
            onChange={(e) => set("value", { ...(value?.value ?? {}), min: e.target.value })}
          />
          <input
            type="number"
            placeholder="Max"
            value={value?.value?.max ?? ""}
            onChange={(e) => set("value", { ...(value?.value ?? {}), max: e.target.value })}
          />
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
          <input
            ...
            onChange={(e) => set("value", { ...(value?.value ?? {}), startDate: e.target.value })}
          />
          <input
            type="date"
            value={value?.value?.endDate ?? ""}
            onChange={(e) => set("value", { ...(value?.value ?? {}), endDate: e.target.value })}
          />
        </div>
      ) : (
